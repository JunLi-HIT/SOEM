   ...
   if (ec_init(ifname))
   {
      ...
      if ( ec_config_init(FALSE) > 0 )
      {
         ...
         if (ec_slave[0].state == EC_STATE_OPERATIONAL )
         {
            ...
            for(i = 1; i <= 1000; i++)
            {
               set_output_int32(1, 0, i);

               ec_send_processdata();
               wkc = ec_receive_processdata(EC_TIMEOUTRET);

               if (i % 100 == 0) {
                  printf("input 0: 0x%x   0x%x   0x%x  0x%x  0x%x \n", get_input_int32(1, 0), get_input_int32(1, 1),
                         get_input_int32(1, 2), get_input_int32(1, 3), get_input_int32(1, 4));

                  set_output_int32(1, 1, mled);
                  set_output_int32(1, 2, ~mled);
                  set_output_int32(1, 3, ~mled);
                  mled = ~mled;
               }
...
//            set_output_int16(1, 0, 0x3);
//            osal_usleep(500000);
//            set_output_int16(1, 0, 0x0);
//            osal_usleep(500000);

//            printf("input dword 0: 0x %x \n", get_input_int32(1, 0));
         }
      }
